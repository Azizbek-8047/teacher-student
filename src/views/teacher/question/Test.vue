...
export default {
  ...
  methods: {
    createTest() {
      this.$store.state.question_test.push({
        id:
          this.questionTest.length > 0
            ? this.questionTest[this.questionTest.length - 1].id + 1
            : 1,
        title: this.form.title,
        optionA: { title: this.form.optionA.title, option: this.form.optionA.option },
        optionB: { title: this.form.optionB.title, option: this.form.optionB.option },
        optionC: { title: this.form.optionC.title, option: this.form.optionC.option },
        optionD: { title: this.form.optionD.title, option: this.form.optionD.option },
      }),
        (this.form.title = ""),
  ...
  computed: {
    ...mapGetters(["getTest", "getTestCount"]),
    offset() {
      return (this.page - 1) * this.limit;
    },
    questionTest() {
      return this.$store.state.question_test;
    },
    list() {
      return this.getTest.slice(this.offset, this.offset + this.limit);
    },
    pages() {
      return Math.ceil(this.getTestCount / this.limit);
    },
  },
  watch: {
    "form.optionA.option"(newValue) {
      if (newValue == true) {
        this.form.optionB.option = false;
        this.form.optionC.option = false;
        this.form.optionD.option = false;
      }
    },
    "form.optionB.option"(newValue) {
      if (newValue == true) {
        this.form.optionA.option = false;
        this.form.optionC.option = false;
        this.form.optionD.option = false;
      }
    },
    "form.optionC.option"(newValue) {
      if (newValue == true) {
        this.form.optionA.option = false;
        this.form.optionB.option = false;
        this.form.optionD.option = false;
